...
if robotMiningSite.settings.botsmk then
	...
	if data.raw["item"]["titanium-plate"] then
	  add_ingredient("mining-robot-3", {"titanium-plate", 2})
	else
	...
	if data.raw["item"]["nitinol-plate"] then
	  add_ingredient("mining-robot-4", {"nitinol-plate", 2})
	else
	  add_ingredient("mining-robot-4", {"steel-plate", 5})
	end

  if data.raw.item["rtg"] and data.raw.technology["rtg"] then
    bobmods.lib.recipe.add_ingredient("mining-robot-5", {"rtg", 1})
    bobmods.lib.tech.add_prerequisite("mining-robot-5", "rtg")
  elseif data.raw.item["vehicle-fusion-cell-1"] and data.raw.technology["vehicle-fusion-cell-equipment-1"] then
    bobmods.lib.recipe.add_ingredient("mining-robot-5", {"vehicle-fusion-cell-1", 1})
    bobmods.lib.tech.add_prerequisite("mining-robot-5", "vehicle-fusion-cell-equipment-1")
  else
    bobmods.lib.recipe.add_ingredient("mining-robot-5", {"fusion-reactor-equipment", 1})
    bobmods.lib.tech.add_prerequisite("mining-robot-5", "fusion-reactor-equipment")
  end
	...
	if data.raw["item"]["diamond-5"] then
	  add_ingredient("mining-robot-5", {"diamond-5", 2})
	else
	  add_ingredient("mining-robot-5", {"steel-plate", 10})
	end
end