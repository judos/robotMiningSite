...
function item_get_basic_type(name)
...
function item_basic_item(inputs)
  local item = {}
  ...
  if inputs.type then
    ...
    item.type = item_get_basic_type(item.name)
  end
...
function item_combine(item1_in, item2_in)
  local item = {}
  local item1 = item_item(item1_in)
  local item2 = item_item(item2_in)
...
function item_add(list, item_in) --increments amount if exists
  local item = item_item(item_in)
  local addit = true
  for i, object in pairs(list) do
    if object[1] == item.name or object.name == item.name then
      addit = false
      list[i] = item_combine(object, item)
    end
  end
  if addit then table.insert(list, item) end
end

function item_add_new(list, item_in) --ignores if exists
  local item = item_item(item_in)
  local addit = true
  for i, object in pairs(list) do
    if item.name == item_basic_item(object).name then addit = false end
  end
  if addit then table.insert(list, item) end
end

function item_remove(list, item)
